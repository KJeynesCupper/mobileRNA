...
Lets look at the plot and the data:
```{r, fig.show="hold"}
# view plot 
...
The `RNAdicercall()` function is used to define the dicer class of a sRNA 
cluster based on the consensus across (specific) replicates. 
...
sRNA_data_summary <- RNAdicercall(data = sRNA_data,  
                                 tidy=TRUE)
# plot results 
plot_distribution <- RNAdistribution(data = sRNA_data_summary, 
                        style = "consensus")
# View figure
...
sRNA_DESeq2 <- RNAdifferentialAnalysis(data = sRNA_data_summary,
                              group = groups,
                              method = "DESeq2")

## Differential analysis using the edgeR method 

sRNA_edgeR <- RNAdifferentialAnalysis(data = sRNA_data_summary,
                              group = groups,
                              method = "edgeR")
```
...
### Plot a volcano plot 
A Volcano plots can be plotted to show the relationship between fold change and 
the adjusted p-value (i.e. FDR) across the dataset. Each point represents an 
individual sRNA cluster. Pointes are coloured according to their significance 
level thresholds (0.05, 0.01, and 0.001 or unchanged). 
...
unique_treatment <- RNApopulation(data = sRNA_data_summary, 
...
unique_control <- RNApopulation(data = sRNA_data_summary, 
...
p3 <- plotHeatmap(data = unique_treatment, row.names = FALSE)
...
Here we can compare the difference and similarities between the treatment 
and controls using a heatmap: 
```{r}
p3$plot
```
...
are tools, such as `TargetFinder` or `sRNATargetDigger`, which use a predictive 
algorithm to identify the most likely locations in the genome where the sRNAs 
might target. 

Lets export the data frame :
```{r, eval=FALSE}

write.table(target_sequences, "./data/mobile_sequences.txt")
...
instead it returns either the absolute number or percentage of sRNA clusters 
