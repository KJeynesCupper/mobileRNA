...
For the following examples, a semi-synthetic small RNA-seq data set has been utilised
to simulate the movement of sRNA molecules from an Tomato (**Solanium lycopersicum**)
rootstock to a Eggplant (**Solanium melongena**) scion, a grafting
system known to be compatible.The three heterograft replicates, known as "heterograft_1",
"heterograft_2" and "heterograft_3" are individual tomato replicates spiked with 
150 tomato sRNA clusters. While the selfgraft replicates, known as "selfgraft_1",
"selfgraft_2" and "selfgraft_3", are the individual tomato replicates without 
the spiked tomato sRNA clusters. Moreover, the replicates mirror each other where,
for instance, "heterograft_1" and "selfgraft_1" are the same replicate, either with 
or without the spiked clusters. These replicates serve for the aim to analyse 
sRNA mobility. 
...
RNA consensus results formed by the function [RNAconsensus()]. While, the 
...
For the hetero-grafts, where the is a eggplant scion and an tomato rootstock:
* `heterograft_1.fq`
* `heterograft_2.fq`
* `heterograft_3.fq`
...
the statistical values and properties of the sRNAs. To consider, use `statistical=TRUE`
...
```{r, echo=FALSE}
cap7 <- "An example heatmap of the potential mobile RNA molecules Identifed by the `DESeq2` method
A). Class of 24-nt sRNAs B) Class of 21/22-nt sRNAs."
...
In this example, we look at the features which the potentially mobile sRNAs from 
tomato could target in the eggplant genome ie. the destination tissue. Similarly, 
the sRNA targets in the origin ie. tomato genome could also be identified with
this function. 

```{r, eval = FALSE}
# matrix containing percentage of sRNA clusters associated to different genomic
# features
table_features <- RNAfeatures(data = mobile_sRNA_24,
...
data_plus_attributes <- RNAattributes(data = sRNA_data_consensus,
                                      annotation = "./annotation/merged_annotation.gff3")

# add matching attributes to potential mobile 24nt sRNA clusters
data_plus_attributes <- RNAattributes(data = mobile_sRNA_24,
                                               annotation = "./annotation/tomato_annotation.gff3")
```
