...
`mobileRNA` works on systems with `R` and `ShortStack` (>= 4.0) installed.
Please consider that `ShortStack` is not available for Windows, hence, Windows 
users will either need to opt to use a virtual machine or 
...
within the same environment. This is required for the alignment step.
...
Firstly, lets load the example FASTA gene files:
<br>
```{r}
...
Firstly, lets load the example GFF files:
<br>
```{r}
...
Firstly, lets load the example GFF repeat region files:
<br>
```{r}
...
two directories (`1_de_novo_detection` and `2_alignment_results`). 
...
Align samples in the directory to the merged genome: 
...
## Import data
The sRNAseq mapping steps generate a directory (`2_alignment_results`) containing a folder 
...
the 2_alignment_results folder generated by the `sRNAmapper()` function), as well as a vector 
...
## Manual mapping steps with `ShortStack`
If the user is unable to undertake the analysis using the `sRNAmapper()` 
...
Here we identify and build a list sRNA-producing loci within each sample to 
assist the alignment to ensure consistency across the analysis. This step
...
``` bash
ShortStack \
--readfile <selfgraft_1.fastq> \
--genomefile <merged_reference.fa> \
--threads 6 \
--mmap u \
--nohp \
--outdir <./output/directory/1_de_novo_detection>
...
dir <- "[./output/directory/1_de_novo_detection]"
...
--readfile <selfgraft_1.fastq> \
--genomefile <merged_reference.fa> \
--locifile <./output/directory/locifile.txt> \
--threads 6 \
--mmap u \
--nohp \
--mincov 5 \
--outdir <./output/directory/2_alignment_results/>
