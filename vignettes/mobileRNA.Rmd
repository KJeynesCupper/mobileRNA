...
informed decisions and better place the sRNA within the genomes(s). Here, we 
propose a method which has been utilised for the analysis of hybrid species 
previously - aligning samples to a merging genome. A merged genome is a single 
FASTA file containing multiple genome assemblies. For system which contains
sRNAs from potentially different genotypes, this allows the algorithm to 
unambiguously predict its genome of origin. However, to be able to understand 
which genome the molecules originate from each genome needs to be 
distinguishable within the merged genome. `mobileRNA` offers a function a tool
to do just that. It merged two genome assemblies into a single file and is 
...
After importing the data into R using RNAimport(), the second core step is the 
consensus dicer classification (see function mobileRNA::RNAdicercall()). 
For a given sRNA cluster, within each sample a sRNA classification is predicted. 
This information is used to define the classification of the cluster based on 
the consensus across replicates. There is a range of parameters to control how 
these are drawn, making it either stringent or loose, which will alter the 
downstream analysis. 
...
mobileRNA::RNAdifferentialAnalysis()). The function outputs additional columns to your 
working dataset, including the raw count mean, log2FoldChange, p-value 
...
molecules gained or losted. 
<br><br><br>
```{r, fig.align="centre", echo=FALSE,out.width="700", out.height="750",fig.cap="The analysis pipeline for mobileRNA" }
...
They removes punctuation and add a prefix to each chromosome name, unique to each
assembly. By default, the string `A_` is added to the genome supplied 
to `genomeA`/`annotationA` and `B_` is added to the genome supplied to 
`genomeB`/`annotationB`. These can be customized to the users preference.
...
population changes, exploring those which have been gained and/lost due to the 
...
containing with "DicerCall") which will state a number from 20-24 in the data 
which represents the nucleotide length of the small RNA, or, state NA/N which 
...
classification for each small RNA cluster based of the classification by each
sample replicate. 
...
## Identify potential tomato mobile molecules which are statistically significant
...
Target prediction tools such as `psRNATarget` (Xinbin Dai, Zhaohong Zhuang and Patrick 
X. Zhao (2018). psRNATarget: a plant small RNA target analysis server (2017 
release). Nucleic Acids Research. doi: 10.1093/nar/gky316) can be used. For a 
tool like `psRNATarget`, we can manipulate the output of `RNAsequences()` to 
match the input style for target prediction using the following code:
```{r, eval = FALSE, message=FALSE}
...
## Identify sRNA population difference
