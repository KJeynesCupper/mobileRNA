...
abstract: >
  Genomic analysis can be utilised to identify difference between RNA 
  populations between two conditions, both in abundance and absolte production.
  This includes the identification of RNAs which are produced by another 
  organism within a bioloigcal system. For example, RNAs produced by pathogens 
  within a host. Or for our purposes, mobile RNAs produced by a plant grafting 
  partner. Plant grafting is a technique which joina differnt plant species or 
  accession together generating a plant with two distinct genotypes. RNAs have 
  been found to be produced by one grafting partner and travel across the graft
  junction to the other grafting partner. Locating and identifying these mobile 
  or forign RNAs including messenger RNA or small RNA, genomic pipelines 
  typically manipulate RNA-seq workflows with consequential mapping to each 
  genome in the system. This comes with benefits and disadvantages. Here, we 
  addess the main disadvantages; the high evels of data noise and false 
  positives. The package mobileRNA provides methods to pre-processs, analyse and 
  visualse the sRNA and mRNA populations. This vignette explains the use of the 
  package and demonstrates quick or advanced workflows. 
output:
...
RNAs, however, it is clear that there is not a standardised approach. To 
identify RNA population which are mobile or native, analysts currently adopt a 
general RNA sequencing pipeline. However, we believe these approaches may have 
several limitations. Here we introduce the R package, `mobileRNA`, a recommended 
pipeline and analysis workflow for the identification of mobile sRNAs/mRNA. 
In addition, the flexibility supports standard sRNA analysis between treatment
and control conditions. For example, to identify sRNA population changes due to
the application of a treatment such as cold/heat stress or exposure to a pest. 
`mobileRNA` ultimately assists pre-processing and analysis including the 
characterization of different populations, visualization of the results, and 
supports output for functional analysis. 
...
found matching the genome of the other grafting partner, this could empirically 
demonstrate it's movement across the graft junction.
...
analysis of plant hybrids [@Lopez-Gomollon2022], including an alignment step 
performed simultaneously on both genomes involved. The rational of this approach 
considers that alignment tools already implement an algorithm ideal for 
identification of the best matches (accordingly to set parameters) in a given 
genome reference, but they do not account for potential matches to DNA sequences 
which are not provided as reference. Therefore, the two genomes from all 
partners involved in the system are merged in a single FASTA file and used as 
reference for the unique alignment. Ultimately, in a bid to supply the algorithm 
with as much information as possible to make the best possible predictions and 
placement of sequencing reads to each genome.

The summarised workflow is shown below 
[(Figure 1)](https://github.com/KJeynesCupper/mobileRNA/blob/main/man/figures/mobileRNA_graphic_1.png) 
where it contains a core RNA analysis and a mobile sRNA/mRNA analysis. The core 
analysis represents the standard workflow for the identification of native 
population difference, for example the sRNA population difference between 
treatment and condition samples, or similarly in a chimeric system, such 
as plant grafting, we might want to explore the native sRNA population from the
sample tissue origin (ie. leaf) which have been lost or gained or changes in 
sRNA abundance. While the mobile analysis, represents the workflow for the 
identification of putative mobile sRNAs or mRNA in a plant graft system.
...
<br><br>
```{r, fig.align="centre", echo=FALSE,out.width="850", out.height="600" }
...
```
<br>
**Figure 2: Comprehensive diagram of complete mobileRNA workflows.**
...
related, it is unexpected that unique reads aligned to the mobile
genotype will be found in the control samples. With that in mind, we can assume
RNAs with reads mapped to the mobile genotype from the control 
...
After alignment of sequencing reads using `mapRNA()`, the results are importing 
into R using `RNAimport()`. For core sRNA analysis, the consensus dicercall 
classification (see function `RNAdicercall()`). For a given sRNA cluster, within 
each sample a sRNA classification is predicted. This information is used to 
define the classification of the cluster based on the consensus across 
replicates, which is added as an additional column to the dataframe. As well a 
further column which states the number of replicates which contributed to the 
consensus class. There is a range of parameters to control how these are drawn, 
making it either stringent or loose, which may alter the downstream analysis. 
These are mainly related to handling ties. 

The next core step for either sRNA or mRNA analysis involves the use of 
differential analysis. Any mobile RNAs will have a significant abundance and 
hence, this information will instruct the user on mobile RNAs which are the most
or least abundant. This step is not highly important to the analysis of mobile 
RNAs as the statistically analysis defining mobile RNAs has already been 
conducted at alignment by the mapping tools. While, for the exploration of 
population dynamics this is much more important step to include, as it defines 
the variation in RNA abundance and also absolute production between two 
conditions. Here we include a function which can be instructed to undertake 
differential analysis using either the `edgeR` [@edgeR_GLM] or `DESeq2` 
[@Love2014] method (see function `RNAdifferentialAnalysis()`).  The function 
outputs additional columns to your working dataset, including the raw count 
mean, log2FoldChange, p-value and adjusted p-value. 
...
the controls. Moreover, the RNA abundance can be explored by summerising the
statistical inference, illustrating the number of RNAs with an significantly 
increased or decreased abundance in the comparision between two conditions. 

For mobile RNA analysis, we introduce the `RNAmobile()` function. It contains 
parameter which can either result in stringent or looser outcomes by utilises 
the number of replicates which contributed to the consensus dicercall 
classification (sRNA) or which contribute to the raw read count (mRNA) as a 
threshold value. For instance, it would be more stringent to select sRNA 
clusters which were unambiguously classified across condition replicates. 

At the end of the analysis, the user will have generated a dataframe
...
will locate mobile RNAs produced by tomato roots which have traveled into the 
...
## sRNA Analysis: locate the sRNA mobilome in a plant grafting experiment
This is a quick-start example analysis of sRNAseq data to locate putative mobile
sRNAs from a plant grafting experiment between eggplant and tomato. Where the 
aim is to locate putative sRNAs, produced by the tomato rootstock, which have 
traveled across the graft junction into the eggplant leaf samples. 
...
use `mmap = n` to exclude multi-mapped reads. We exclude multimappers as we 
...
For a given sRNA cluster, each replicate has a predicted dicercall,
also known as the sRNA class, based on the length in nucleotides of the
most abundant sRNA. Depending on your organism these lengths will vary, for 
plants these are between 20-24 nucleotides and support predictions of their 
sRNA class, such as microRNA or small interfering RNA. The consensus dicercall
identifies the most likely sRNA class based on either all samples or 
specifically named samples. The output can be used as threshold values for 
downstream analysis, and to remove data noise depending on data quality.
...
Here, users can undertake differential analysis of sRNA within the experimental 
design to explore changes in abundance. The function allows for two methods;
`edgeR` or `DESeq`.

Ensure that the working dataframe is correctly ordered to ensure proper control
verses treatment statistical analysis. See `RNAreorder()` for more information.
...
### 6. Identify Potential Mobile sRNA clusters
...
### 7. Output
At the end of this analysis the user produces a data frame where rows represent
potential mobile sRNA clusters and columns include information on the cluster, 
individual sample replicates, and more: 
...
## mRNA Analysis: locate the mRNA mobilome in a plant grafting experiment
This is a quick-start example analysis of mRNAseq data to locate putative mobile
mRNAs from a plant grafting experiment between eggplant and tomato. Where the 
aim is to locate putative mRNAs, produced by the tomato rootstock, which have 
traveled across the graft junction into the eggplant leaf samples. 

### 1. Alignment 

Here, the user aligns mRNA sequencing reads to the merged genome using a 
HISAT2 [@Kim2015] and HTSeq [@Anders2014] pipeline wrapped by the `mapRNA()` 
function. This pipeline excludes multimapped reads from analysis, as there is 
currently no implemented method to distinguish reads mapped to multiple 
locations within either one or both of the genomes in the merged reference. 
...
sampleData <- data.frame(sample = c("selfgraft_1", "selfgraft_2", 
                        ...
                        mate1 = c("selfgraft_mRNAdemo_1.fq", 
                                  "selfgraft_mRNAdemo_2.fq", 
                                  "heterograft_mRNAdemo_1.fq", 
                                  "heterograft_mRNAdemo_2.fq"))
...
### 3. Import example data 
Import a more comprehensive dataset for the downstream analysis:
```{r}
data("mRNA_data")
```
...
Undertake differential analysis of mRNA within the experimental design
to explore changes in mRNA abundance. Here we utilise the DESeq2 analytical 
method. 


```{r, message=FALSE}
# sample conditions in order within dataframe
groups <- c("Selfgraft", "Selfgraft", "Heterograft", "Heterograft")
...
How about summarizing the mRNA population which are statistically
significant:
...
### 5. Identify Potential Mobile mRNA Molecules

Select the putative mobile mRNA clusters using `RNAmobile()`. This
requires supplying the function with a unique identifier of the
rootstock genome (ie. mobile genome), which is the prefix "B" to the tomato 
chromosome names. Here, the function retains mRNA molecules which were aligned
...
At the end of this analysis the user produces a data frame where rows represent
potential mobile mRNA molecules and columns include information on the 
following:
...
different groups. Here we undertake quality control using three different 
methods:

- Distribution of sRNA classes
- Principal Component Analysis (PCA)
- Distance matrix using hierarchical clustering heatmap
...
with length between 20-24 nucleotides, and the lengths signify the sRNA class 
and specific functional role in (epi)genetic regulation. If there was an 
...
```{r, fig.cap=cap1, fig.show="hold", fig.height=7, fig.width=15}
# View plot (only)
sample_distribution_line$plot
```
<br>
```{r, fig.cap=cap2, fig.show="hold", fig.height=7, fig.width=15}
# View plot (only)
sample_distribution_bar$plot
```
<br>
```{r, fig.cap=cap3, fig.show="hold", fig.height=7, fig.width=15}
# View plot
...
because PC1 is showing the variation between the control self-graft sample and 
the simulated heterograft sample while PC2 shows the natural variation between 
the replicates. 
...
Have a look at the `sRNA_data` dataframe, you will see that for each sample the
...
cluster by each sample within the analysis. There are several
...
classes at `random` or defining the cluster as unclassified through the 
`exclude` option. However, when there is a tie between the choice of a class or 
...
Differential analysis is undertaken to identify sRNAs which 
...
users have the option to use the `DESeq2` [@Love2014] or `edgeR` [@edgeR_GLM] 
analytical method.
...
**mobileRNA** offer three different tools to assist the functional analysis. 
The first, can be used to identify genomic features associated with the sRNA 
producing loci associated to each cluster. This function is known as 
`RNAattributes()`. The second tool does something very similar, it identifies 
the same overlaps but returns a value stating either the absolute or relative 
number of sRNA clusters which over overlap with specific genomic features. 
This function is known as `RNAfeatures()`. Finally, the last tool, called 
`RNAsequences()` calculates the most common sRNA sequence for each candidate 
and calculates the potential target DNA sequence. 
...
region of 1 kb upstream and downstream of the each gene. 

```{r, eval=FALSE}
annotation_file <- system.file("inst/extdata", 
                                    "prefix_reduced_chr2_Tomato.gff",
                                    package="mobileRNA")
...
regions.The results can either be displayed in the dataframe as an 
...
between the most common RNA sequences within a cluster the user can utilise the 
`duplicates` parameter to choose whether to set the RNA sequence as NA or 
...
required to share the RNA sequence to count as a match. This is supplied to the 
...
## Manual sRNA Alignement Pipeline
If users wish to undertake the alignment step independantly, the manual steps 
...
for (i in samples) {
  file_path <- file.path(dir, i, "Results.gff3")
...
produce our results. The output folders are used as imput for the `RNAimport()`
function.
...
bowtie-build --threads 6 [genomefile.fa] [genomefile]
...
*bash scripting*
```{bash, eval=FALSE}
hisat2-build --threads 6 [genomefile.fa] [genomefile]
```
...
-U [fastqfile_1.fq] | samtools view -bS | samtools sort -o [outputfile.bam]
...
--type=mRNA \
--idattr=Name \
[outputfile.bam] [annotationfile.gff]
```
...
-1 [fastqfile_1.fq] \
-2 [fastqfile_2.fq] \
| samtools view -bS | samtools sort -o [outputfile.bam]
