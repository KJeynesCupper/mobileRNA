...
sRNA-producing genes in parallel analysis. Similarly, for mRNA data sets, a 
volcano plot can be used. 
...
For sRNA data, `ShortStack` (>= 4.0) is requires. Please consider that 
`ShortStack` is not available for Windows, hence, Windows users will either need 
to opt to use a virtual machine or 
...
for installation with Conda. This will ensure all dependencies are available
within the same environment. 

For mRNA data, `HISTAT2`, `HTSeq`, `SAMtools` are required within the same 
Conda environment. Please refer to \link{https://anaconda.org/bioconda/hisat2},
...
Hence, we also provide a more comprehensive data set, called `sRNA_data`, 
which stores an example dataframe produced by the importation step with 
`RNAimport()`. This can be loaded in the R environment by using the following 
command:
...
fasta_1_url <- file.path(url_remote, "chr101112_Eggplant_V4.1.fa")
fasta_2_url <- file.path(url_remote,"chr123_S_lycopersicum_chromosomes.4.00.fa")
...
anno1_url<-file.path(url_remote,"Eggplant_V4.1_function_IPR_final.gff")
anno2_url<-file.path(url_remote,"ITAG4.1_release/ITAG4.1_gene_models.gff")
...
## 3. Merging Genome Annotations with Repetitive Regions (optional)
For downstream analysis, we may wish to see how many RNA molecules overlap with
repetitive regions in genome(s). 
```{r}
# Construct URL to example GFF files
repeats1_url <-file.path(urlFTP_1,"Eggplant_V4.1_repeats.gff3")
repeats2_url <-file.path(urlFTP_2,"ITAG4.0_release/ITAG4.0_REPET_repeats_aggressive.gff")
...
# define temporary output directory 
output_annotationRR_file<- file.path(tempfile("merged_annotation_RR", 
                                              fileext = ".gff3"))

# merge annotation files into a single file
merged_repeatregions <-RNAmergeAnnotations(annotationA = repeats1,
                                           annotationB = repeats2,
                                        output_file = output_annotationRR_file)
```
...
sampleData <- data.frame(sample = c("selfgraft_1", "selfgraft_2", 
                                     "selfgraft_3", "heterograft_1", 
                                     "heterograft_2", "heterograft_3"),
                        mate1 = c("selfgraft_1_1.fq", "selfgraft_2_1.fq", 
                                   "selfgraft_3_1.fq", "heterograft_1_1.fq", 
                                    "heterograft_2_1.fq", "heterograft_3_1.fq"),
                         mate2 = c("selfgraft_1_2.fq", "selfgraft_2_2.fq", 
                                    "selfgraft_3_2.fq", "heterograft_1_2.fq", 
                                    "heterograft_2_2.fq", "heterograft_3_2.fq"))
...
Here we will demonstrate sRNA analysis with the pre-loaded data. 
```{r}
...
## Plot a volcano diagram for mRNA data 
...
## Manual sRNA mapping steps with `ShortStack`
If the user is unable to undertake the analysis using the `mapRNA()` 
function. The manual steps are shown below, of which, will need modifying 
to suit your files and directory names. 

### Core sRNA analysis
#### Step 1 - De novo sRNA analysis with `ShortStack`
...
Collate all the sRNA loci information from each sample into a text file. 
```{r, eval=FALSE}
...
gff_alignment <- GenomicRanges::GRangesList()
  ...
  loci_out <- file.path(output_locifile,"locifile.txt")
...
### Mobile sRNA analysis
#### Step 1 - De novo sRNA analysis with `ShortStack`
First map with Bowtie, to exclude multi-mapped reads. 
```{bash, eval=FALSE}
...
## Manual mRNA mapping steps with `HISAT2` and `HTSeq`
If the user is unable to undertake the analysis using the `mapRNA()` 
function. The manual steps are shown below, of which, will need modifying 
to suit your files and directory names. 
