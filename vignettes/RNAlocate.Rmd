---
title: "Overview of the RNAlocate R package for mobile sRNA identification between genotypes"
...
[RNAlocate::RNAgenomeMerge()] function. As default, this function changes the 
...
```{r, eval = FALSE}
RNAmergeGenome(genomeA = "/Users/user1/projectname/workplace/reference/ref1.fa",
...
If you want to know what genomic features are associated with the locus of a sRNA  
we will need to overlap the genomic annotation of our reference genomes with the 
clusters annotation file. The genomic annotation of our reference genomes need to
firstly be merged into a single file using the `RNAannotationMerge` function. 
Similar to the `RNAgenomeMerge` function, as default, it also changes the 
chromosome names. Make sure these mirror the changes made in the genome reference

The overlap between the annotation and the loci of clusters can be computed 
using the `RNAannotationOverlap`. 
