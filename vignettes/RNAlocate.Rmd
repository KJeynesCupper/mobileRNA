...
devtools::install_github("KJeynesCupper/RNAlocate", ref = "devel")
```
...
documentation. For some functions there are recurring augments. 
...
Similarly, the augment `out` instructs the function of the location to save the 
output file(s).  In the analysis steps, some important augments include `samples` 
...
which refers to the control samples in the analysis. A recurring augment in the 
...
The function *create_workplace* will produce the key directories which will store 
...
in your working directory. Replace the `URL` augment with the FTP URL of a 
...
The same as before, replace the `URL` augment with the online FTP URL of 
...
Use `chr_info()` to produce a text file containing the chromosome names and 
sizes, and use `chr_modify()` to alter the names of the chromosomes in a genome. 
...
chr_info(genome = "/Users/user1/projectname/workplace/reference/merge/
           ref_merged.fasta", 
           out =  "/Users/user1/projectname/workplace/reference/merge/")
# Remove punctuation from merged reference genome.  
chr_modify(genome = "/Users/user1/projectname/workplace/reference/merge/
           Solanum_lycopersicum.fa.gz", 
          old = "SL4.",
          new = "SL4")
```

Use the function `merge_files()` to concatenate the reference genome files into a 
...
To check the quality, use the `quality_check()` function which performs
...
### Step 2 
The second step in mapping is undertaken in R, use the `cluster_info.` 
...
single file using the `merge_files` function. Then, the overlap between the
files can be computed using the `find_overlap` function. This function, similar
to the `merge_files` functions, commands a system call to the operating system 
...
results can be plotted as either a bar chart or a line graph for each sample 
and can be shown in a bar chart facet or in as a single line graph, where each 
line represents a sample. Alternatively, the results can be plotted individually 
...
```{r, echo=FALSE}
cap9 <- "An example bar plot, in a facet format, to show the distribution of 
...
sample.distribution <- RNAdistribution(sRNA_data, style = "line")

```

## Define The Class of Each sRNA  
...
calculation on (see augment `conditions`). It is recommend to state the treatment 
...
## Plot consensus classification of clusters

Use `frequency_plot()` to visualize the distribution of the consensus sRNA 
...
can plot the absolute or relative frequency, use augment `relative`. It can be 
benefical to view the absolute frequency as well as the relative to observe
whether the data meets your expectations. 

Similarly, the `distribution_plot()` function can be used to plot the distribution 
of sRNA classes within each sample, individually. 

```{r, echo=FALSE}
cap1 <- "A box plot example using `frequency_plot()` function with the example 
data. In this example, the frequency of each small RNA class consensus is 
shown."

cap2 <- "A box plot example using the `distribution_plot()` function with the 
example data. In this example, the frequency of each small RNA class  
is illustrated for each sample"

```

```{r, message=FALSE, fig.cap=cap1}
frequency_plot(sRNA_data_summary, relative = FALSE)
```

```{r , message=FALSE, fig.cap=cap2, eval = FALSE}
## ERROR ON THIS PLOT. 
distribution_plot(sRNA_data_summary)
```

## Count preparation with DESeq2 or edgeR 
...
The data must be prepared for the DE analysis using the `DE_prepare()`, and the 
steps taken depend on the analytical method chosen. 
In the package, this has been simplified into a single
function where you must only state which method to you. Here we breakdown what 
happens in each method: 

**DESeq2 method**
The `DESeq2` method creates a DESeqDataSet object from a table of 
un-normalised counts and estimates the size factors using the 
"median ratio method" to normalise the counts based on 
sequencing differences in library size and RNA composition of the samples. 

**edgeR method**
The `edgeR` method creates DGEList object from a table of un-normalised counts,
and TMM normalization is applied to data set to account for compositional 
...
## DESeq prepare for each subset.
# 24-nt subset
sRNA_24_prep_DESeq <- DE_prepare(sRNA_24, group, method = "DESeq2")
# 21/22-nt subset
sRNA_2122_prep_DESeq <- DE_prepare(sRNA_2122, group, method = "DESeq2")



## edgeR prepare for each subset.
# 24-nt subset
sRNA_24_prep_edgeR <- DE_prepare(sRNA_24, group, method = "edgeR")
# 21/22-nt subset
sRNA_2122_prep_edgeR <- DE_prepare(sRNA_2122, group, method = "edgeR")
```


## PCA and Heatmap plots to illustrate sample distances 
`PCA_plot()` and `distance_plot()` functions visualize the variation between the 
samples in an experiment to access sample distance as part of a quality check 
of the data. 

`The PCA_plot()` function undertakes a Principal Component Analysis 
(PCA) using an unbiased log transformation and plots the results. 
PCA is a method used to highlight variation and reduces the dimensions in a 
data set to make it easier to visualise the similarities and difference.

Similarly,The PCA_plot()` function undertakes hierarchical clustering with an 
unbiased log transformation to calculate sample distance. 

### Quality control plots for 24-nt sRNA dataset
```{r, echo=FALSE}
cap3 <- "An example of the `DESeq2` method, illustracting the 
 24-nt sRNA data set. A) PCA plot. B) Hierarchical clustering heatmap"

cap4 <- "An example of the `edgeR` method, illustracting the 
 24-nt sRNA data set. A) PCA plot. B) Hierarchical clustering heatmap"

cap5 <- "An example of the `DESeq2` method, illustracting the 
21/22-nt sRNA data set. A) PCA plot. B) Hierarchical clustering heatmap"

cap6 <- "An example of the `edgeR` method, illustracting the 
21/22-nt sRNA data set. A) PCA plot. B) Hierarchical clustering heatmap"

```

```{r ,message=FALSE, fig.cap=cap3, fig.show="hold", out.width="50%", fig.subcap= c("A","B")}

PCA_plot(sRNA_24_prep_DESeq)
distance_plot(sRNA_24_prep_DESeq)

```

```{r, message=FALSE, fig.cap=cap4, fig.show="hold", out.width="50%", fig.subcap= c("A","B")}
PCA_plot(sRNA_24_prep_edgeR)
distance_plot(sRNA_24_prep_edgeR)


```


### Quality control plots for 21/22-nt sRNA dataset  

```{r, message=FALSE, fig.cap=cap5, fig.show="hold", out.width="50%", fig.subcap= c("A","B")}

PCA_plot(sRNA_2122_prep_DESeq)
distance_plot(sRNA_2122_prep_DESeq)

#gridExtra::grid.arrange(p5, p6, nrow(1))
```

```{r, message=FALSE, fig.cap=cap6, fig.show="hold", out.width="50%", fig.subcap= c("A","B")}
PCA_plot(sRNA_2122_prep_edgeR)
distance_plot(sRNA_2122_prep_edgeR)
...
`DE_analysis()` allows the user to run differential expression analysis on a 
data set using either `DESeq2` or `edgeR`.The results from the analysis are 
not easily accessible and do not contain the information on each sRNA,therefore, 
the results are added to the summary data set using the `DE_results()` 
function. This enables easier viewing and manipulation of the data. 

The results from the anal
```{r, message=FALSE}
...
the `mobile_molecules()` function. It selects clusters mapping to the rootstock 
...
The `mobile_molecules()` function extracts the most statistically significant 
...
sRNA_24_mobile_DEseq <- mobile_molecules(data = res_sRNA_24_DESeq2, 
                                         controls = control_names, 
                                         id = "SL40", 
                                         task = "remove")

# remove clusters associated to tomato in 2122nt RNA database 
sRNA_2122_mobile_DEseq  <- mobile_molecules(data = res_sRNA_2122_DESeq2,
                                            controls = control_names, 
                                            id = "SL40", 
                                            task = "remove")
...
p10 <- heatmap_plot(sRNA_24_mobile_DEseq)
p11 <- heatmap_plot(sRNA_24_mobile_edgeR)
...
p12 <- heatmap_plot(sRNA_2122_mobile_DEseq)
p13 <- heatmap_plot(sRNA_2122_mobile_edgeR)
```
