...
fig_width: 7
vignette: >
 %\VignetteIndexEntry{mobileRNA_chimeric}
...
classification. This analysis step uses this information to 
...
either result in stringent or looser outcomes. These parameter utilises the 
...
`RNAgenomeMerge()` function. This removes punctuation from chromosome
...
23-nt, or 24-nt in this data. For example:
...
[mobileRNA::RNAmergeAnnotations()] function can be utilised to alter both 
...
functions to assist with functional analysis can also be utilised to pull 
...
on the heterograft samples rather than the full dataset (self-grafts + heterograft
replicates) as all potential mobile sRNAs should have zero counts or RPM value in 
the self-grafts control replicates. By including the full dataset in these 
