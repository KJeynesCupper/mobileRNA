---
title: "Overview: RNAlocate for mobile sRNA identification between genotypes"
...
# reference 2
download.file(url = 'https://solgenomics.net/ftp/tomato_genome/Heinz1706/assembly/build_4.00/S_lycopersicum_chromosomes.4.00.fa.gz',
              destfile = "/Users/user1/projectname/workplace/reference/Solanum_lycopersicum.fa.gz",
              method = "wget")
```
...
```{r, eval = FALSE}
# reference 1
download.file(url = 'dir.annoation1.gff', 
              destfile = "/Users/user1/projectname/workplace/annotation/Solanum_melongena_anno.gff", 
              method = "wget")

# reference 2
download.file(url = 'dir.annoation2.gff', 
              destfile = "/Users/user1/projectname/workplace/annotation/Solanum_lycopersicum_anno.gff", 
              method = "wget")
```
...
reads and reads outside the desired lengths (16-30 nucleotides). The 
`checkQuality()` function can be used again to check the quality of the trimmed
samples to ensure the cleaning has worked as designed. 
...
multi-mapping) to the merged genome. 
...
calculations. It requires two different directory paths; one is the path to 
...
RNA clusters, stored in the argument `clusters`. 
As an optional extra, if you wish to find the overlap between the clusters and 
the merged genome reference annotations, created in the pre-mapping stage, 
this can be assigned to the argument `features`. 
...
of the mapping reference genomes, the chromsome names will need to match in the 
annotation files(s). 
