...
#'
#' @description Calculates the percentage of sRNA dicer-derived clusters which
...
RNAfeatures <- function(data, annotation, repeats = NULL, percentage = TRUE){
  ...
  if(is.null(repeats)) {
    # features
    TEs<-subset(annotation_info, type=="transposable_element",
                type=="transposable_element_gene")
    } else {
    repeats_info < rtracklayer::import(repeats)
      TEs<-subset(repeats_info, type=="transposable_element",
                  type=="transposable_element_gene")
    }
  ...
  five_UTR<-subset(annotation_info[IRanges::overlapsAny(annotation_info,
                                                          ...
                                                          ignore.strand=TRUE)],
                                                         type=="five_prime_UTR")
  five_UTR<-GenomicRanges::setdiff(five_UTR, c(TEs), ignore.strand=TRUE)
  three_UTR<-subset(annotation_info[IRanges::overlapsAny(annotation_info,
                                                               genes,
                                                          ignore.strand=TRUE)],
                                                       type=="three_prime_UTR")
  three_UTR<-GenomicRanges::setdiff(three_UTR, c(five_UTR, TEs),
                                    ignore.strand=TRUE)
  exons<-subset(annotation_info[IRanges::overlapsAny(annotation_info,
                                                           genes,
                                                           ignore.strand=TRUE)],
                type=="exon")
  exons<-GenomicRanges::setdiff(exons, c(five_UTR, three_UTR, TEs),
                                ignore.strand=TRUE)
  introns <- GenomicRanges::setdiff(genes, c(exons, five_UTR, three_UTR),
                                    ignore.strand=TRUE)

  promoters <- genes
 GenomicRanges::start(promoters[BiocGenerics::strand(promoters) == "+"]) <-
   GenomicRanges::start(promoters[BiocGenerics::strand(promoters) == "+"])-2000

  GenomicRanges::end(promoters[BiocGenerics::strand(promoters) == "+"]) <-
    GenomicRanges::start(promoters[BiocGenerics::strand(promoters) == "+"]) + 0

  GenomicRanges::start(promoters[BiocGenerics::strand(promoters) == "-"]) <-
    GenomicRanges::end(promoters[BiocGenerics::strand(promoters) == "-"]) - 0

  GenomicRanges::end(promoters[BiocGenerics::strand(promoters) == "-"]) <-
    GenomicRanges::end(promoters[BiocGenerics::strand(promoters) == "-"]) + 2000
  ...
  others <-   BiocGenerics::setdiff(subset(annotation, type=="chromosome"),
                                    c(TEs, exons,five_UTR,three_UTR,introns,
