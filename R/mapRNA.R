...
#' on the installation of `ShortStack` (>= 4.0), `bowtie` with Conda.
#' Please note that `ShortStack` is only compatible with Linux and Mac
#' operating systems.
#'
...
#'
#' The function invokes a number of OS commands, and is dependent
#' on the installation of `HISAT2`,`HTSeq`, `grep` and `SAMtools` with `Conda.`
...
#' alignment file (BAM) and raw counts file ("Results.txt"). Note, that the
...
#'       output_dir = output_location,
#'       genomefile = GenomeRef,
#'       annotationfile = GenomeGFF,
#'       sampleData = sampleData,
#'        condaenv = "/Users/user-name/miniconda3/envs/rnaseq",
#'        python = "/Users/user-name/miniconda3/envs/rnaseq/bin/python")
#' }
#'
 ...
 contain additional file types. This location must only contain fastq files.")
  ...
  if(input == "sRNA"){
    ...
    #   }
      ...
      mRNA_map(sampleData = sampleData,
